...
const Calendario = () => {
  ...
  const fetchProyectos = useCallback(async () => {
    setLoading(true);
    try {
      const [proyRes, empRes] = await Promise.all([
        supabase
          .from('proyectos')
          .select(`
            *,
        ...
        supabase.from('empleados').select('id, nombre_completo, fecha_nacimiento, google_calendar_cumple_id').eq('activo', true).order('nombre_completo'),
      ]);
      ...
      console.error('Error cargando proyectos para calendario:', error);
      toast({
        variant: 'destructive',
        title: 'Error',
    ...
    <>
      ...
      <div className="flex flex-col h-[calc(100vh-80px)] min-h-0">
        <div className="shrink-0 mb-2">
          ...
          <p className="text-gray-600 mt-1 text-sm">Vista de proyectos por fechas. Filtra por estatus en el panel izquierdo.</p>
        </div>
        {loading ? (
          ...
          <div className="flex-1 min-h-0">
            <CalendarView proyectos={proyectos} empleados={empleados} />
          </div>
        )}
      </div>
    </>
