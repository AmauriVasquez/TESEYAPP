...
import { formatDateForPrint, formatDateTable } from '@/lib/dateUtils';
...
const FormatoPedido = ({ pedidoData, onPrint, onDownloadJson }) => {
  ...
  const tipoPedido = (pedidoData?.tipo_pedido ?? 'material') === 'activo' ? 'activo' : 'material';
    ...
    <div className="space-y-4">
      <div ref={printRef} className="p-8 border rounded-lg bg-white text-black">
        ...
        <section className="grid grid-cols-3 gap-4 mt-4 text-sm">
          ...
          <div><span className="font-bold">Fecha:</span> {pedidoData.fecha ? formatDateTable(pedidoData.fecha) : 'N/A'}</div>
          ...
          <table className="w-full text-sm border-collapse border border-gray-400">
            ...
            <tbody>
              {items.map((item, index) => (
                <tr key={index}>
                  <td className="border border-gray-300 p-2">{descripcionImpresionPedidoItem(item, tipoPedido)}</td>
                  <td className="border border-gray-300 p-2 text-center">{item.cantidad}</td>
                  <td className="border border-gray-300 p-2 text-center">{unidadImpresionPedidoItem(item)}</td>
